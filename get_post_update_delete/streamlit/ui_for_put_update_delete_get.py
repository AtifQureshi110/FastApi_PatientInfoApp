...
# # Fetch a Specific Book by Index (GET)
# st.header("Fetch a Book by Index")
# index = st.number_input("Enter Book Index (0, 1, 2, ...)", min_value=0, step=1, key="fetch_index")
...
#             )
#         else:
#             st.warning("There is no book with this index number.")
#     else:
#         st.error(f"Error: {response.status_code} - {response.text}")
